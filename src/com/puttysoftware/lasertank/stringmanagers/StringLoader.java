...
public class StringLoader {
    ...
    public static void setDefaultLanguage() {
        ...
        StringLoader.LANGUAGE_NAME = StringLoader.loadLanguageString(
                StringConstants.LANGUAGE_STRINGS_FILE, StringLoader.LANGUAGE_ID)
                + "/";
    ...
    public static String[] loadLocalizedLanguagesList() {
        if (StringLoader.LOCALIZED_LANGUAGES == null) {
            ...
            try (final InputStream is = StringLoader.LOAD_CLASS
                    .getResourceAsStream(StringLoader.LOAD_PATH
                            + StringLoader.LANGUAGE_NAME + filename);
                ...
                CommonDialogs.showErrorDialog(
                        "Something has gone horribly wrong trying to load the language data!",
                        "FATAL ERROR");
                LaserTank.logErrorDirectly(ioe);
            }
        }
        return StringLoader.LOCALIZED_LANGUAGES
                .toArray(new String[StringLoader.LOCALIZED_LANGUAGES.size()]);
    }

    private static String loadLanguageString(final int fileID,
            final int strID) {
        ...
        String value = StringLoader.LANGUAGE_STRING_CACHE.get(fileID)
                .get(Integer.valueOf(strID));
        if (value == null) {
            // Cache string file
            StringLoader.cacheLanguageStringFile(fileID);
            // Get correct value
            value = StringLoader.LANGUAGE_STRING_CACHE.get(fileID)
                    .get(Integer.valueOf(strID));
        }
        return value;
    }
    ...
    public static String loadString(final int fileID, final int strID) {
        if (fileID < 0) {
            return StringLoader.loadLanguageString(
                    -StringConstants.NOTL_STRINGS_FILE, strID);
        } else {
            if (StringLoader.STRING_CACHE == null) {
                // Create string cache
                StringLoader.STRING_CACHE = new ArrayList<>();
            }
            if (StringLoader.STRING_CACHE.size() <= fileID
                    || StringLoader.STRING_CACHE.get(fileID) == null) {
                // Cache string file
                StringLoader.cacheStringFile(fileID);
            }
            String value = StringLoader.STRING_CACHE.get(fileID)
                    .get(Integer.valueOf(strID));
            if (value == null) {
                // Cache string file
                StringLoader.cacheStringFile(fileID);
                // Get correct value
                value = StringLoader.STRING_CACHE.get(fileID)
                        .get(Integer.valueOf(strID));
            }
            return value;
        }
    ...
    private static void cacheStringFile(final int fileID) {
        ...
        try (final InputStream is = StringLoader.LOAD_CLASS.getResourceAsStream(
                StringLoader.LOAD_PATH + StringLoader.LANGUAGE_NAME + filename
                        + StringConstants.STRINGS_EXTENSION);
                ...
                if (line != null) {
                    ...
                    if (StringLoader.STRING_CACHE.size() <= fileID
                        ...
                        for (int x = 0; x <= fileID; x++) {
                            if (StringLoader.STRING_CACHE.size() <= x
                                    || StringLoader.STRING_CACHE
                                            .get(x) == null) {
                                StringLoader.STRING_CACHE
                                        .add(new HashMap<>());
                            }
                    ...
                    StringLoader.STRING_CACHE.get(fileID).put(
                            Integer.valueOf(Integer.parseInt(splitLine[0])),
                            splitLine[1]);
                }
            }
        } catch (final IOException ioe) {
    ...
    private static void cacheLanguageStringFile(final int fileID) {
        ...
        try (final InputStream is = StringLoader.LOAD_CLASS
                .getResourceAsStream(StringLoader.LOAD_PATH + filename
                        + StringConstants.STRINGS_EXTENSION);
                final ResourceStreamReader rsr = new ResourceStreamReader(is)) {
            String line = StringConstants.COMMON_STRING_EMPTY;
            while (line != null) {
                ...
                if (line != null) {
                    ...
                    final String[] splitLine = line.split(" = ");
                    if (StringLoader.LANGUAGE_STRING_CACHE.size() <= fileID
                            || StringLoader.LANGUAGE_STRING_CACHE
                                    .get(fileID) == null) {
                        // Entry for string file doesn't exist, so create it
                        for (int x = 0; x <= fileID; x++) {
                    ...
                    StringLoader.LANGUAGE_STRING_CACHE.get(fileID).put(
                            Integer.valueOf(Integer.parseInt(splitLine[0])),
                            splitLine[1]);
                }
            }
            ...
            CommonDialogs.showErrorDialog(
                    "Something has gone horribly wrong trying to load the language string data!",
                    "FATAL ERROR");
            LaserTank.logErrorDirectly(ioe);
        }
    }
