...
public abstract class AbstractArenaObject extends CloneableObject {
    ...
    private final boolean hasDirection() {
        return this.direction != Direction.INVALID
                && this.direction != Direction.NONE;
    }
    ...
    public Direction laserEnteredAction(final int locX, final int locY,
        ...
        if (this.isSolid()) {
            if (forceUnits > this.getMinimumReactionForce() && this.canMove()) {
                try {
                    final AbstractArenaObject nextObj = LaserTank
                            .getApplication().getArenaManager().getArena()
                            .getCell(locX + dirX, locY + dirY, locZ,
                                    this.getLayer());
                    final AbstractArenaObject nextObj2 = LaserTank
                            .getApplication().getArenaManager().getArena()
                            .getCell(locX + dirX * 2, locY + dirY * 2, locZ,
                                    this.getLayer());
                    if (this instanceof AbstractMovableObject && nextObj != null
                            && nextObj instanceof AbstractMovableObject
                            && nextObj.canMove()
                            && (nextObj2 != null
                                    && !nextObj2.isConditionallySolid()
                                    || forceUnits > 2)) {
                        ...
                        LaserTank.getApplication().getGameManager()
                                .updatePushedPositionLater(locX, locY, dirX,
                                        dirY, gmo, locX + dirX, locY + dirY,
                                        gmo2, laserType,
                                        forceUnits - Math.max(1, this
                                                .getMinimumReactionForce()));
                    } else {
                ...
                final AbstractArenaObject adj = LaserTank.getApplication()
    ...
    public final String getBaseImageName() {
        return StringLoader.loadString(StringConstants.IMAGE_STRINGS_FILE,
                this.getStringBaseID());
    }
