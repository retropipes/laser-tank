...
public class ArenaManager {
    ...
    public static int showSaveDialog() {
        ...
        if (mode == Application.STATUS_EDITOR) {
            ...
            source = StringLoader.loadString(
        ...
        if (frame != null) {
            frame.getRootPane().putClientProperty(
                    StringLoader.loadString(StringConstants.NOTL_STRINGS_FILE,
                            StringConstants.NOTL_STRING_WINDOW_MODIFIED),
    ...
    public boolean loadArenaDefault() {
        try {
            return this.loadArenaImpl(ArenaManager.class.getProtectionDomain()
        ...
        if (saved) {
            ...
            if (file != null && dir != null) {
                ...
                if (extension.equals(Extension.getArenaExtension())) {
                    ...
                    this.scoresFileName = ArenaManager
                            .getNameWithoutExtension(file);
                    ArenaManager.loadFile(filename, false, false);
                } else if (extension
                        .equals(Extension.getProtectedArenaExtension())) {
                    this.lastUsedArenaFile = filename;
                    this.scoresFileName = ArenaManager
                            .getNameWithoutExtension(file);
    ...
    private static void loadFile(final String filename,
            final boolean isSavedGame, final boolean protect) {
        if (!FilenameChecker.isFilenameOK(ArenaManager.getNameWithoutExtension(
                ArenaManager.getFileNameOnly(filename)))) {
    ...
    public boolean saveArena(final boolean protect) {
        ...
        if (app.getMode() == Application.STATUS_GAME) {
            if (this.lastUsedGameFile != null && !this.lastUsedGameFile
                    .equals(StringConstants.COMMON_STRING_EMPTY)) {
                final String extension = ArenaManager
                        .getExtension(this.lastUsedGameFile);
                if (extension != null) {
                    if (!extension.equals(Extension.getGameExtension())) {
                        this.lastUsedGameFile = ArenaManager
            ...
            if (protect) {
                if (this.lastUsedArenaFile != null && !this.lastUsedArenaFile
                        .equals(StringConstants.COMMON_STRING_EMPTY)) {
                    final String extension = ArenaManager
                            .getExtension(this.lastUsedArenaFile);
                    if (extension != null) {
                        if (!extension.equals(
                                Extension.getProtectedArenaExtension())) {
                            this.lastUsedArenaFile = ArenaManager
                                    .getNameWithoutExtension(
                                            this.lastUsedArenaFile)
                                    + Extension
                                            .getProtectedArenaExtensionWithPeriod();
                        }
                    } else {
                        this.lastUsedArenaFile += Extension
                                .getProtectedArenaExtensionWithPeriod();
                    }
                    ArenaManager.saveFile(this.lastUsedArenaFile, false,
                            protect);
                } else {
                    return this.saveArenaAs(protect);
                }
    ...
    public boolean saveArenaAs(final boolean protect) {
        ...
        String fileOnly = StringLoader.loadString(
        ...
        while (!FilenameChecker.isFilenameOK(fileOnly)) {
            ...
            if (file != null && dir != null) {
                ...
                if (!FilenameChecker.isFilenameOK(fileOnly)) {
                    CommonDialogs.showErrorDialog(StringLoader.loadString(
                            StringConstants.DIALOG_STRINGS_FILE,
                            StringConstants.DIALOG_STRING_ILLEGAL_CHARACTERS),
