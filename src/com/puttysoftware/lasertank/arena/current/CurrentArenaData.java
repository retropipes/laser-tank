...
public final class CurrentArenaData extends AbstractArenaData {
    ...
    private LowLevelFlagDataStore dirtyData;
    ...
    public CurrentArenaData() {
        this.data = new LowLevelArenaDataStore(AbstractArenaData.MIN_COLUMNS,
                AbstractArenaData.MIN_ROWS, AbstractArenaData.MIN_FLOORS,
                ArenaConstants.NUM_LAYERS);
        this.virtualData = new LowLevelArenaDataStore(
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_FLOORS,
                ArenaConstants.NUM_VIRTUAL_LAYERS);
        this.fillVirtual();
        this.dirtyData = new LowLevelFlagDataStore(
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_FLOORS);
        this.savedState = new LowLevelArenaDataStore(AbstractArenaData.MIN_ROWS,
                AbstractArenaData.MIN_COLUMNS, AbstractArenaData.MIN_FLOORS,
                ArenaConstants.NUM_LAYERS);
        this.foundX = -1;
        this.foundY = -1;
    ...
    @Override
    public boolean isCellDirty(final AbstractArena arena, final int row,
    ...
    @Override
    public AbstractArenaObject getCell(final AbstractArena arena, final int row,
    ...
    @Override
    public AbstractArenaObject getVirtualCell(final AbstractArena arena,
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    final AbstractArenaObject mo = this.getCell(arena, y, x, z,
                            t.getLayer());
                    if (mo != null) {
                        if (t.equals(mo)) {
    ...
    @Override
    public void tickTimers(final AbstractArena arena, final int floor,
            ...
            for (x = 0; x < this.getColumns(); x++) {
                for (y = 0; y < this.getRows(); y++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        final AbstractArenaObject mo = this.getCell(arena, y, x,
                                floorFix, w);
                        if (mo != null) {
    ...
    @Override
    public void checkForEnemies(final AbstractArena arena, final int floorIn,
            final int enemyLocXIn, final int enemyLocYIn,
        ...
        if (scanE) {
            try {
                final AntiTank at = (AntiTank) this.getCell(arena, this.foundX,
                        this.foundY, floor, template.getLayer());
    ...
    @Override
    public int checkForMagnetic(final AbstractArena arena, final int floor,
            final int centerX, final int centerY, final Direction dir) {
        if (dir == Direction.EAST) {
            return this.linearScanMagnetic(arena, centerX, centerY, floor,
                    Direction.EAST);
    ...
    @Override
    public boolean linearScan(final AbstractArena arena, final int xIn,
        ...
        if (d == Direction.NORTH) {
            final AbstractArenaObject tank = LaserTank.getApplication()
                    .getGameManager().getTank();
                            ...
                            if (obj.isOfType(TypeConstants.TYPE_ANTI)) {
                                final int[] unres = DirectionResolver
                                        .unresolveRelativeDirection(
                                                obj.getDirection());
                                final Direction invert = DirectionResolver
                                        .resolveRelativeDirectionInvert(
                                                unres[0], unres[1]);
                                if (d == invert) {
                                    this.foundX = xFix;
                                    this.foundY = u;
    ...
    @Override
    public int linearScanMagnetic(final AbstractArena arena, final int xIn,
            final int yIn, final int zIn, final Direction d) {
        // Perform the scan
        int xFix = xIn;
        int yFix = yIn;
    ...
    @Override
    public int[] findObject(final AbstractArena arena, final int z,
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                for (int w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        final AbstractArenaObject obj = this.getCell(arena, x,
                                y, z, w);
    ...
    @Override
    public int[] circularScan(final AbstractArena arena, final int xIn,
        ...
        for (u = xFix - r; u <= xFix + r; u++) {
            for (v = yFix - r; v <= yFix + r; v++) {
                for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        final AbstractArenaObject obj = this.getCell(arena, v,
                                u, zFix, w);
                        final AbstractArenaObject savedObj = obj
                                .getSavedObject();
                        String testName;
    ...
    @Override
    public int[] circularScanTunnel(final AbstractArena arena, final int xIn,
            final int yIn, final int zIn, final int r, final int tx,
        ...
        for (u = xFix - r; u <= xFix + r; u++) {
            for (v = yFix - r; v <= yFix + r; v++) {
                ...
                if (v >= 0 && v < AbstractArenaData.MIN_ROWS && u >= 0
                        && u < AbstractArenaData.MIN_COLUMNS) {
                    final AbstractArenaObject obj = this.getCell(arena, v, u,
                            zFix, w);
    ...
    @Override
    public void circularScanRange(final AbstractArena arena, final int xIn,
            final int yIn, final int zIn, final int r, final int rangeType,
        ...
        for (u = xFix - r; u <= xFix + r; u++) {
            for (v = yFix - r; v <= yFix + r; v++) {
                ...
                for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                    try {
                        this.getCell(arena, u, v, zFix, w).rangeAction(xFix,
                                yFix, zFix, u - xFix, v - yFix, rangeType,
                                forceUnits);
                    } catch (final ArrayIndexOutOfBoundsException aioob) {
                        // Do nothing
                    }
    ...
    @Override
    public boolean circularScanTank(final AbstractArena arena, final int x,
            final int y, final int z, final int r) {
        final int[] tankLoc = LaserTank.getApplication().getGameManager()
                .getTankLocation();
        int fX = x;
        int fY = y;
        int fZ = z;
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                for (int z = 0; z < this.getFloors(); z++) {
                    final AbstractArenaObject obj = this.getCell(arena, y, x, z,
                            ArenaConstants.LAYER_LOWER_OBJECTS);
                    if (obj instanceof AntiTank) {
                        // Kill the tank
                        final GameManager gm = LaserTank.getApplication()
                                .getGameManager();
                ...
                for (int z = 0; z < this.getFloors(); z++) {
                    ...
                    if (!(obj instanceof Ground)) {
                        ...
                        LaserTank.getApplication().getGameManager()
                                .morph(obj.changesToOnExposure(
                                        MaterialConstants.MATERIAL_ICE), y, x,
                                        z, ArenaConstants.LAYER_LOWER_GROUND);
                    }
                }
            }
        }
    ...
    @Override
    public void fullScanAllButtonOpen(final AbstractArena arena, final int zIn,
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            ...
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                ...
                final AbstractArenaObject obj = this.getCell(arena, y, x, zFix,
                        source.getLayer());
        ...
        if (flag) {
            ...
            if (!(this.getCell(arena, dx, dy, zFix,
                    source.getLayer()) instanceof Ground)) {
                this.setCell(arena, new Ground(), dx, dy, zFix,
                        source.getLayer());
    ...
    @Override
    public void fullScanAllButtonClose(final AbstractArena arena, final int zIn,
            final AbstractButton source) {
        // Perform the scan
        int zFix = zIn;
        if (arena.isThirdDimensionWraparoundEnabled()) {
        ...
        if (flag) {
            ...
            if (!this.getCell(arena, dx, dy, zFix, source.getLayer()).getClass()
                ...
                this.setCell(arena, source.getButtonDoor(), dx, dy, zFix,
                        source.getLayer());
    ...
    @Override
    public void fullScanButtonBind(final AbstractArena arena, final int dx,
            final int dy, final int zIn, final AbstractButtonDoor source) {
        // Perform the scan
        int z = zIn;
        if (arena.isThirdDimensionWraparoundEnabled()) {
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                final AbstractArenaObject obj = this.getCell(arena, x, y, z,
                        source.getLayer());
                if (obj instanceof AbstractButton) {
                    final AbstractButton button = (AbstractButton) obj;
                    if (source.getClass()
                            .equals(button.getButtonDoor().getClass())) {
                        button.setDoorX(dx);
                        button.setDoorY(dy);
    ...
    @Override
    public void fullScanButtonCleanup(final AbstractArena arena, final int px,
            final int py, final int zIn, final AbstractButton button) {
        // Perform the scan
        int zFix = zIn;
        if (arena.isThirdDimensionWraparoundEnabled()) {
            zFix = this.normalizeFloor(zFix);
        ...
        for (int x = 0; x < AbstractArenaData.MIN_COLUMNS; x++) {
            for (int y = 0; y < AbstractArenaData.MIN_ROWS; y++) {
                ...
                final AbstractArenaObject obj = this.getCell(arena, x, y, zFix,
                        button.getLayer());
                if (obj instanceof AbstractButton) {
                    if (((AbstractButton) obj).boundButtonDoorEquals(button)) {
                        this.setCell(arena, new Ground(), x, y, zFix,
                                button.getLayer());
                    }
                }
            }
    ...
    @Override
    public void fullScanFindButtonLostDoor(final AbstractArena arena,
    ...
    @Override
    public void setCell(final AbstractArena arena, final AbstractArenaObject mo,
            final int row, final int col, final int floor, final int layer) {
        int fR = row;
        int fC = col;
        int fF = floor;
    ...
    @Override
    public void markAsDirty(final AbstractArena arena, final int row,
            final int col, final int floor) {
        int fR = row;
        int fC = col;
        int fF = floor;
    ...
    @Override
    public void setVirtualCell(final AbstractArena arena,
            ...
            for (int col = 0; col < this.getColumns(); col++) {
                for (int floor = 0; floor < this.getFloors(); floor++) {
                    for (int layer = 0; layer < ArenaConstants.NUM_VIRTUAL_LAYERS; layer++) {
                        this.setVirtualCell(arena, new Empty(), row, col, floor,
                                layer);
                    }
                }
            }
                ...
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.savedState.setCell(
                                this.getCell(arena, y, x, z, w).clone(), x, y,
                                z, w);
                    }
                }
            }
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.setCell(arena,
                                (AbstractArenaObject) this.savedState
                                        .getCell(x, y, z, w).clone(),
                                y, x, z, w);
                    }
                }
            }
    ...
    @Override
    public void resize(final AbstractArena arena, final int zIn,
        ...
        final LowLevelArenaDataStore tempStorage = new LowLevelArenaDataStore(y,
                x, z, ArenaConstants.NUM_LAYERS);
            ...
            for (v = 0; v < x; v++) {
                for (w = 0; w < z; w++) {
                    for (t = 0; t < ArenaConstants.NUM_LAYERS; t++) {
                        try {
                            tempStorage.setCell(this.getCell(arena, v, u, w, t),
                                    u, v, w, t);
                        } catch (final ArrayIndexOutOfBoundsException aioob) {
                            // Do nothing
                        }
        ...
        this.virtualData = new LowLevelArenaDataStore(x, y, z,
                ArenaConstants.NUM_VIRTUAL_LAYERS);
        this.dirtyData = new LowLevelFlagDataStore(x, y, z);
        // Fill any blanks
    ...
    @Override
    public void resizeSavedState(final int z,
            final AbstractArenaObject nullFill) {
        final int x = AbstractArenaData.MIN_ROWS;
        final int y = AbstractArenaData.MIN_COLUMNS;
        // Allocate temporary storage array
            ...
            for (v = 0; v < x; v++) {
                for (w = 0; w < z; w++) {
                    for (t = 0; t < ArenaConstants.NUM_LAYERS; t++) {
                        try {
                            tempStorage.setCell(
                                    this.savedState.getCell(v, u, w, t), u, v,
                                    w, t);
                        } catch (final ArrayIndexOutOfBoundsException aioob) {
                            // Do nothing
                        }
    ...
    @Override
    public void fillNulls(final AbstractArena arena,
                    ...
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        if (this.savedState.getCell(y, x, z, w) == null) {
                            if (w == ArenaConstants.LAYER_LOWER_GROUND) {
                                ...
                                this.savedState.setCell(new Empty(), y, x, z,
                                        w);
                            }
                        }
                    }
    ...
    @Override
    public void writeData(final AbstractArena arena, final XDataWriter writer)
            throws IOException {
        int y, x, z, w;
    ...
    @Override
    public AbstractArenaData readData(final AbstractArena arena,
    ...
    private static CurrentArenaData readDataG1(final AbstractArena arena,
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    final AbstractArenaObject obj = LaserTank.getApplication()
                            .getObjects().readArenaObjectG2(reader, ver);
    ...
    private static CurrentArenaData readDataG2(final AbstractArena arena,
            final XDataReader reader, final int ver) throws IOException {
        int y, x, z, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
    ...
    private static CurrentArenaData readDataG3(final AbstractArena arena,
            final XDataReader reader, final int ver) throws IOException {
        int y, x, z, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    final AbstractArenaObject obj = LaserTank.getApplication()
                            .getObjects().readArenaObjectG3(reader, ver);
                    lt.setCell(arena, obj, y, x, z, obj.getLayer());
                }
            }
        }
    ...
    private static CurrentArenaData readDataG4(final AbstractArena arena,
            final XDataReader reader, final int ver) throws IOException {
        int y, x, z, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    final AbstractArenaObject obj = LaserTank.getApplication()
                            .getObjects().readArenaObjectG4(reader, ver);
                    lt.setCell(arena, obj, y, x, z, obj.getLayer());
                }
            }
        }
    ...
    private static CurrentArenaData readDataG5(final AbstractArena arena,
            final XDataReader reader, final int ver) throws IOException {
        ...
        for (x = 0; x < arenaSizeX; x++) {
            for (y = 0; y < arenaSizeY; y++) {
                for (z = 0; z < arenaSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        lt.setCell(arena,
                                LaserTank.getApplication().getObjects()
                                        .readArenaObjectG5(reader, ver),
                                y, x, z, w);
                    }
                }
            }
    ...
    private static CurrentArenaData readDataG6(final AbstractArena arena,
            final XDataReader reader, final int ver) throws IOException {
        int y, x, z, w, arenaSizeX, arenaSizeY, arenaSizeZ;
        arenaSizeX = reader.readInt();
        arenaSizeY = reader.readInt();
    ...
    public void writeSavedState(final XDataWriter writer) throws IOException {
        ...
        for (x = 0; x < this.getColumns(); x++) {
            for (y = 0; y < this.getRows(); y++) {
                for (z = 0; z < this.getFloors(); z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        ((AbstractArenaObject) this.savedState.getCell(y, x, z,
                                w)).writeArenaObject(writer);
                    }
                }
            }
        }
    ...
    @Override
    public void readSavedState(final XDataReader reader,
    ...
    private void readSavedStateG2(final XDataReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
        saveSizeZ = reader.readInt();
        this.savedState = new LowLevelArenaDataStore(saveSizeY, saveSizeX,
                saveSizeZ, ArenaConstants.NUM_LAYERS);
            ...
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    this.savedState.setCell(
                            ...
                            y, x, z, ArenaConstants.LAYER_LOWER_GROUND);
                }
            }
        }
        if (saveSizeX != AbstractArenaData.MIN_COLUMNS
                || saveSizeY != AbstractArenaData.MIN_ROWS) {
            this.resizeSavedState(saveSizeZ, new Ground());
        }
    }

    private void readSavedStateG3(final XDataReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
    ...
    private void readSavedStateG4(final XDataReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
    ...
    private void readSavedStateG5(final XDataReader reader,
            final int formatVersion) throws IOException {
        ...
        for (x = 0; x < saveSizeX; x++) {
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.savedState.setCell(LaserTank.getApplication()
                                .getObjects().readArenaObjectG5(reader,
                                        formatVersion),
                                y, x, z, w);
                    }
                }
    ...
    private void readSavedStateG6(final XDataReader reader,
            final int formatVersion) throws IOException {
        int y, x, z, w, saveSizeX, saveSizeY, saveSizeZ;
        saveSizeX = reader.readInt();
        saveSizeY = reader.readInt();
        ...
        for (x = 0; x < saveSizeX; x++) {
            for (y = 0; y < saveSizeY; y++) {
                for (z = 0; z < saveSizeZ; z++) {
                    for (w = 0; w < ArenaConstants.NUM_LAYERS; w++) {
                        this.savedState.setCell(LaserTank.getApplication()
                                .getObjects().readArenaObjectG6(reader,
                                        formatVersion),
                                y, x, z, w);
                    }
                }
    ...
    public void updateUndoHistory(final HistoryStatus whatWas) {
        try {
            this.iue.updateUndoHistory(
                    (LowLevelArenaDataStore) this.data.clone(), whatWas);
        } catch (final CloneNotSupportedException cnse) {
            LaserTank.logError(cnse);
        }
    }
    ...
    private class ImageUndoEngine {
        ...
        public void updateUndoHistory(final LowLevelArenaDataStore newImage,
                final HistoryStatus newWhatWas) {
            this.undoHistory.push(newImage, newWhatWas);
        }

        public void updateRedoHistory(final LowLevelArenaDataStore newImage,
                final HistoryStatus newWhatWas) {
        ...
        private class HistoryEntry {
            ...
            HistoryEntry(final LowLevelArenaDataStore i,
                    final HistoryStatus hww) {
                this.histImage = i;
        ...
        private class HistoryStack {
            ...
            public void push(final LowLevelArenaDataStore i,
                    final HistoryStatus hww) {
