...
public class ImageManager {
    ...
    public static BufferedImageIcon getImage(final AbstractArenaObject obj,
    ...
    static BufferedImageIcon getUncachedImage(final AbstractArenaObject obj,
            final boolean useText) {
        try {
            ...
            final URL url = ImageManager.LOAD_CLASS
                    .getResource(ImageManager.LOAD_PATH
                            + StringLoader.loadString(
                                    StringConstants.NOTL_STRINGS_FILE,
                                    StringConstants.NOTL_STRING_OBJECTS_SUBPATH)
                            + extraPath + normalName
                            + StringConstants.COMMON_STRING_NOTL_IMAGE_EXTENSION_PNG);
            ...
            if (useText && customText != null) {
                if (ImageManager.DRAW_FONT == null) {
                    try (InputStream is = ImageManager.class
                            .getResourceAsStream(StringLoader.loadString(
                                    StringConstants.NOTL_STRINGS_FILE,
                                    StringConstants.NOTL_STRING_FONT_PATH)
                                    + StringLoader.loadString(
                                            StringConstants.NOTL_STRINGS_FILE,
                                            StringConstants.NOTL_STRING_FONT_FILENAME))) {
                        final Font baseFont = Font
                                .createFont(Font.TRUETYPE_FONT, is);
                        ImageManager.DRAW_FONT = baseFont
                                .deriveFont(ImageManager.DRAW_SIZE);
                    } catch (final Exception ex) {
                        ImageManager.DRAW_FONT = Font
                                .decode(ImageManager.DRAW_FONT_FALLBACK);
                    }
                }
                final Graphics2D g2 = image.createGraphics();
                g2.setFont(ImageManager.DRAW_FONT);
                g2.setColor(obj.getCustomTextColor());
                g2.drawString(customText, ImageManager.DRAW_HORZ,
                        ImageManager.DRAW_VERT);
            }
    ...
    private static BufferedImageIcon getTransformedTunnel(final int cc,
            final boolean useText) {
        try {
            final BufferedImageIcon icon = ImageCache
                    .getCachedImage(new Tunnel(), useText);
            Color color;
    ...
    public static BufferedImageIcon getCompositeImage(
            final AbstractArenaObject obj1, final AbstractArenaObject obj2,
            final boolean useText) {
    ...
    private static BufferedImageIcon getCompositeImageDirectly(
    ...
    public static BufferedImageIcon getVirtualCompositeImage(
            final AbstractArenaObject obj1, final AbstractArenaObject obj2,
            final AbstractArenaObject... otherObjs) {
        BufferedImageIcon result = ImageManager.getCompositeImage(obj1, obj2,
                true);
