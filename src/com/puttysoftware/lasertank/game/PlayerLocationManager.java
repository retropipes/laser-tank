...
public final class PlayerLocationManager {
	...
	this.playerData = new LowLevelNumberDataStore(ArenaConstants.PLAYER_DIMS, ArenaConstants.NUM_PLAYERS);
	this.playerData.fill(-1);
	this.savedPlayerData = new LowLevelNumberDataStore(ArenaConstants.PLAYER_DIMS, ArenaConstants.NUM_PLAYERS);
	this.savedPlayerData.fill(-1);
	this.savedRemoteData = new LowLevelNumberDataStore(ArenaConstants.PLAYER_DIMS, ArenaConstants.NUM_PLAYERS);
	...
	this.savedPlayerData = (LowLevelNumberDataStore) this.playerData.clone();
    }

    void restorePlayerLocation() {
	this.playerData = (LowLevelNumberDataStore) this.savedPlayerData.clone();
    }

    void saveRemoteLocation() {
	this.savedRemoteData = (LowLevelNumberDataStore) this.playerData.clone();
    }

    void restoreRemoteLocation() {
	this.playerData = (LowLevelNumberDataStore) this.savedRemoteData.clone();
    }
}
