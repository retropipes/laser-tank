...
class ScoreTracker {
    ...
    private static final String MAC_PREFIX = StringLoader.loadString(
            StringConstants.NOTL_STRINGS_FILE,
            StringConstants.NOTL_STRING_DIRECTORY_UNIX_HOME);
    private static final String WIN_PREFIX = StringLoader.loadString(
            StringConstants.NOTL_STRINGS_FILE,
            StringConstants.NOTL_STRING_DIRECTORY_WINDOWS_APPDATA);
    private static final String UNIX_PREFIX = StringLoader.loadString(
            StringConstants.NOTL_STRINGS_FILE,
            StringConstants.NOTL_STRING_DIRECTORY_UNIX_HOME);
    private static final String MAC_DIR = StringLoader.loadString(
            StringConstants.NOTL_STRINGS_FILE,
            StringConstants.NOTL_STRING_DIRECTORY_SCORES_MAC);
    private static final String WIN_DIR = StringLoader.loadString(
            StringConstants.NOTL_STRINGS_FILE,
            StringConstants.NOTL_STRING_DIRECTORY_SCORES_WINDOWS);
    private static final String UNIX_DIR = StringLoader.loadString(
            StringConstants.NOTL_STRINGS_FILE,
    ...
    boolean checkScore() {
        if (this.trackScores) {
            return this.ssMgr.checkScore(
                    new long[] { this.moves, this.shots, this.others });
        } else {
            return false;
        }
    }
    ...
    void commitScore() {
        if (this.trackScores) {
            final boolean result = this.ssMgr.addScore(
                    new long[] { this.moves, this.shots, this.others });
            if (result) {
    ...
    void setScoreFile(final String filename) {
        ...
        if (this.trackScores) {
            ...
            if (this.trackScores) {
                ...
                this.ssMgr = new SavedScoreManager(3, 10,
                        ScoreManager.SORT_ORDER_DESCENDING, 10000L,
                        ...
                        new String[] { StringLoader.loadString(
                                StringConstants.GAME_STRINGS_FILE,
    ...
    private static File getScoresFile(final String filename) {
        ...
        b.append(LaserTank.getApplication().getArenaManager().getArena()
                .getActiveLevelNumber() + 1);
