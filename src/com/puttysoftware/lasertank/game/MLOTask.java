...
final class MLOTask extends Thread {
    ...
    public MLOTask() {
        this.setName(StringLoader.loadString(StringConstants.NOTL_STRINGS_FILE,
                StringConstants.NOTL_STRING_MLOH_NAME));
    ...
    void activateMovement(final int zx, final int zy) {
        ...
        if (zx == 2 || zy == 2 || zx == -2 || zy == -2) {
            ...
            this.sy = zy;
            this.magnet = false;
            GameManager.updateUndo(false, false, false, true, false, false,
            ...
            GameManager.updateUndo(false, false, false, false, true, false,
                    false, false, false, false);
        } else {
            // Moving normally
    ...
    void activateFrozenMovement(final int zx, final int zy) {
        ...
        this.sx = zx;
        this.sy = zy;
        GameManager.updateUndo(false, false, false, false, false, false, false,
                false, false, false);
        this.move = true;
        if (!gm.isReplaying()) {
            gm.updateReplay(false, zx, zy);
    ...
    void activateObjects(final int zx, final int zy, final int pushX,
    ...
    void activateLasers(final int zx, final int zy, final int zox,
            final int zoy, final int zlt, final AbstractArenaObject zshooter) {
            ...
            do {
                try {
                    ...
                    for (final MovingLaserTracker tracker : this.laserTrackers) {
                        if (tracker.isTracking()) {
                            tracker.trackPart1(
                                    this.areObjectTrackersTracking());
                        }
                    }
                    // Abort check 2
                    if (this.abort) {
                    ...
                    int actionType = 0;
                    if (this.move) {
                        if (!this.magnet && Math.abs(this.sx) <= 1
                                && Math.abs(this.sy) <= 1) {
                    ...
                    for (final MovingLaserTracker tracker : this.laserTrackers) {
                        if (tracker.isTracking()) {
                            this.mover = tracker.trackPart2(this.sx, this.sy,
                                    this.mover);
                        }
                    }
                    for (final MovingObjectTracker tracker : this.objectTrackers) {
                    ...
                    if (this.move) {
                        ...
                        if (objs[ArenaConstants.LAYER_LOWER_OBJECTS]
                                .solvesOnMove()) {
                            this.abort = true;
                            if (this.move) {
                                LaserTank.getApplication().getArenaManager()
                                        .setDirty(true);
                                this.defrostTank();
                                gm.moveLoopDone();
                                this.move = false;
                    ...
                    && (this.loopCheck || this.areLaserTrackersChecking()
                            || this.areObjectTrackersChecking()));
    ...
    private boolean canMoveThere() {
        ...
        try {
            try {
                lgo = m.getCell(px + this.sx, py + this.sy, pz,
                        ArenaConstants.LAYER_LOWER_GROUND);
                ugo = m.getCell(px + this.sx, py + this.sy, pz,
                        ArenaConstants.LAYER_UPPER_GROUND);
                loo = m.getCell(px + this.sx, py + this.sy, pz,
        ...
        return MLOTask.checkSolid(lgo) && MLOTask.checkSolid(ugo)
                && MLOTask.checkSolid(loo) && MLOTask.checkSolid(uoo);
    }
        ...
        if (this.proceed) {
            ...
            try {
                if (this.canMoveThere()) {
                    ...
                    if (m.getCell(px, py, pz,
                            pw) instanceof AbstractMovableObject) {
                    ...
                    px = MLOTask.normalizeColumn(px + this.sx,
                            AbstractArena.getMinColumns());
                    py = MLOTask.normalizeRow(py + this.sy,
                            AbstractArena.getMinRows());
                    gm.getTank().setSavedObject(m.getCell(px, py, pz, pw));
                    ...
                    lgo.moveFailedAction(plMgr.getPlayerLocationX() + this.sx,
                            plMgr.getPlayerLocationY() + this.sy,
                            plMgr.getPlayerLocationZ());
                    if (ugo == null) {
                        ugo = new Ground();
                    }
                    ugo.moveFailedAction(plMgr.getPlayerLocationX() + this.sx,
                            plMgr.getPlayerLocationY() + this.sy,
                            plMgr.getPlayerLocationZ());
                    if (loo == null) {
                        loo = new Ground();
                    }
                    loo.moveFailedAction(plMgr.getPlayerLocationX() + this.sx,
                            plMgr.getPlayerLocationY() + this.sy,
                            plMgr.getPlayerLocationZ());
                    if (uoo == null) {
                        uoo = new Ground();
                    }
                    uoo.moveFailedAction(plMgr.getPlayerLocationX() + this.sx,
                            plMgr.getPlayerLocationY() + this.sy,
                            plMgr.getPlayerLocationZ());
                    if (gm.getTank().getSavedObject()
                            .isOfType(TypeConstants.TYPE_MOVER)) {
                        final Direction dir = gm.getTank().getSavedObject()
                                .getDirection();
                        final int[] unres = DirectionResolver
                                .unresolveRelativeDirection(dir);
                        this.sx = unres[0];
                ...
                m.setCell(gm.getTank(), plMgr.getPlayerLocationX(),
                        plMgr.getPlayerLocationY(), plMgr.getPlayerLocationZ(),
                        pw);
    ...
    private boolean checkLoopCondition(final boolean zproceed) {
        ...
        return zproceed && (!lgo.hasFriction() || !ugo.hasFriction()
                || this.mover || this.frozen) && this.canMoveThere();
    }
    ...
    static boolean checkSolid(final int zx, final int zy) {
        final GameManager gm = LaserTank.getApplication().getGameManager();
        final AbstractArenaObject next = LaserTank.getApplication()
                .getArenaManager().getArena().getCell(zx, zy,
